...
if __name__ == "__main__":
    res = 0
    for i, (lights, buttons, joltages) in enumerate(
        parse_stream(sys.stdin.readlines())
    ):
        B = [z3.Int(f"b{b}") for b in range(len(buttons))]
        equations = [b >= 0 for b in B]
        for j, joltage in enumerate(joltages):
            equation = 0
            for b, button in enumerate(buttons):
                if j in button:
                    equation += B[b]
            equations.append(equation == joltage)
        s = z3.Optimize()
        s.add(equations)
        s.minimize(z3.Sum(B))
        if s.check() == z3.sat:
            m = s.model()
            button_sum = sum(m[b].as_long() for b in B)
            res += button_sum
        else:
            print("unsat")
    print(res)
